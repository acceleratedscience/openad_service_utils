...
For property prediction, the wrapper uses the `service_type` (e.g., `get_molecule_property`, `get_protein_property`, `get_crystal_property`, `get_mesh_property`) and the `service_name` to look up the appropriate predictor in the `PropertyPredictorRegistry`.
